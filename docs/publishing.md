...
| Item | Owner step |
|---|---|
| **Name** | Rayfold since 2026-09-10; on that date `rayfold` was free on npm, PyPI, crates.io and GitHub, and rayfold.dev did not resolve. **Owner:** run a trademark search before offering anything commercial under the name. `python scripts/rename-project.py rayfold NEWNAME` renames everything in one command if needed. |
...
| **Public repository** | **Owner:** make it public before publishing: npm provenance refuses a private source repository, GitHub Pages on a free plan needs a public one, and so does private vulnerability reporting, which `SECURITY.md` points to. Then turn on Settings > Security > Private vulnerability reporting, and Settings > Pages > Source: GitHub Actions. |
...
| **Maven Central namespace** | **Owner:** the group is `dev.rayfold` (`GROUP` in `kotlin/gradle.properties`). Central verifies it with a DNS record on rayfold.dev, so register that domain; or change `GROUP` to `io.github.eddyboutros`, which Central verifies through GitHub. |
