/**
 * SHA-256 in plain JavaScript, so @rayfold/schema (and the browser client that depends on it) needs neither
 * node:crypto nor Buffer. Checked against node:crypto in canonical.test.ts.
 */
const K = [
...
export function sha256Hex(text: string): string {
  ...
  const blocks = new Uint8Array(((bytes.length + 9 + 63) >> 6) << 6);
  blocks.set(bytes);
  blocks[bytes.length] = 0x80;
  const view = new DataView(blocks.buffer);
  view.setUint32(blocks.length - 8, Math.floor(bytes.length / 0x20000000));
